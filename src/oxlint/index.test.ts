...
const overrides = (config: ReturnType<typeof oxlint>) =>
  (config.overrides ?? []) as unknown as Override[];
...
test('defaults: native plugins, correctness as error, four js plugins, no typeAware option', () => {
  ...
  assert.deepEqual(pluginNames(config.jsPlugins), [
    'stylistic',
    'perfectionist',
    'eslint-js',
    'typescript-js',
...
test('environment and globals live in overrides so extends keeps them', () => {
  const config = oxlint();
  assert.deepEqual(overrideFor(config, '**/*.{js,mjs,cjs,jsx,ts,mts,cts,tsx}')?.env, {
    node: true,
  });
  assert.ok(
    overrideFor(config, '**/*.{ts,mts,cts,tsx}')?.rules?.['typescript-js/naming-convention'],
  );
  assert.equal(
    overrideFor(config, '**/*.{js,mjs,cjs,jsx}')?.rules?.['eslint-js/camelcase'],
    'error',
  );
});
...
test('test framework options add their overrides and plugins', () => {
  ...
  const jestOverride = overrides(jest).find(
    (o) => o.files[0] === 'tests/**/*.{js,mjs,cjs,jsx,ts,mts,cts,tsx}',
  );
  ...
  const mochaOverride = overrides(mocha).find(
    (o) => o.files[0] === 'test/**/*.{js,mjs,cjs,ts,mts,cts}',
  );
  assert.deepEqual(mochaOverride?.env, { mocha: true });
  assert.deepEqual(pluginNames(mochaOverride?.jsPlugins), ['mocha']);
  assert.deepEqual(overrides(mocha).find((o) => o.files[0] === 'test/**/_*')?.rules, {
  ...
  assert.ok(
    overrides(vitest).some(
      (o) => o.plugins?.includes('vitest') && o.rules?.['vitest/no-focused-tests'],
    ),
  );
});
...
test('a11y, reactCompiler and typeChecked toggles', () => {
  ...
  assert.equal(
    overrideFor(oxlint({ react: true }), '**/*.{jsx,tsx}')?.rules?.['react/purity'],
    'off',
  );
  assert.equal(
    overrideFor(oxlint({ react: true, reactCompiler: true }), '**/*.{jsx,tsx}')?.rules?.[
      'react/purity'
    ],
    'error',
  );
  ...
  assert.equal(
    overrideFor(oxlint({ typeChecked: true }), ts)?.rules?.['typescript/no-floating-promises'],
    'error',
  );
});
